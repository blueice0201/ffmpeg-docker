...
export function registerApiRoutes<T extends OpenAPIHono>(app: T) {
    ...
    return c.html(`
      ...
      <html>
        ...
        <body>
          ...
          <h2>Documentation</h2>
          <ul>
            <li><a href="/doc/compose-manifest">Compose Manifest Reference (JSON)</a></li>
  ...
  return appWithReadme.openapi(getEndpointsRoute, (c) => {
    const endpoints = [
      ...
      { path: '/doc/compose-manifest', method: 'GET', description: 'Compose manifest schema reference and examples' },
      { path: '/convert', method: 'POST', description: 'Queue async multi-asset video compose task (sequential or timeline manifest)' },
      { path: '/convert/sequential', method: 'POST', description: 'Queue async sequential video compose task' },
      { path: '/convert/timeline', method: 'POST', description: 'Queue async timeline video compose task' },
      { path: '/convert/{taskId}', method: 'GET', description: 'Get convert task status or download result' }
    ];
